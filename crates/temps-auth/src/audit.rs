...
/// Recorded when an authenticated request is rejected with HTTP 403 by any
/// authorization guard (`permission_guard!`, `project_scope_guard!`,
/// `project_access_guard!`, `project_permission_guard!`, ...), so permission
/// probing by a low-privilege credential leaves a trace.
///
/// Written centrally by the auth middleware on observing a FORBIDDEN
/// response rather than at each guard call site, so every guard — including
/// ones added later — is covered without threading the audit service into
/// the guard macros. The actor is optional because deployment tokens carry
/// no user identity; `auth_source` preserves who/what was denied either way.
#[derive(Debug, Clone, Serialize)]
pub struct PermissionDeniedAudit {
    /// Account behind the denied credential, when it has one.
    pub user_id: Option<i32>,
    /// Credential kind that was denied, e.g. "session", "cli_token",
    /// "api_key:<name>", "deployment_token:<name>", "unauthenticated".
    pub auth_source: String,
    /// HTTP method of the denied request.
    pub method: String,
    /// Request path that was denied (no query string).
    pub path: String,
    pub ip_address: Option<String>,
    pub user_agent: String,
}
...
mod failure_audit_tests {
    ...
    #[test]
    fn test_permission_denied_audit_deployment_token_has_no_actor() {
        let audit = PermissionDeniedAudit {
            user_id: None,
            auth_source: "deployment_token:ci-deploy".to_string(),
        ...
        assert_eq!(audit.actor_user_id(), None);
        let json =
            AuditOperation::serialize(&audit).expect("PermissionDeniedAudit serializes");
        assert!(json.contains("deployment_token:ci-deploy"));
        assert!(json.contains("/projects/7"));
    }

    #[test]
    fn test_permission_denied_audit_user_keeps_actor() {
        let audit = PermissionDeniedAudit {
            user_id: Some(7),
            auth_source: "session".to_string(),
